...
import avatarImg from '../images/photo-avatar.png';
...
Promise.all([api.getUserInfo(), api.getCards()])
  .then(([profileInfo, cardsData]) => {
    ...
    // if we prefer local avatar, reapply it after server data arrives
    try {
      if (userInfo && userInfo._keepLocalAvatar) {
        const avatarEl = document.querySelector('.profile__avatar');
        if (avatarEl && typeof avatarImg !== 'undefined') {
...
// экзепляр класса информации о пользователе (не перезаписывать локальный аватар)
const userInfo = new UserInfo({ nameSelector: '.profile__name', aboutSelector: '.profile__caption', avatarSelector: '.profile__avatar', keepLocalAvatar: true });

// Set default avatar from local file until API provides avatar
const avatarEl = document.querySelector('.profile__avatar');
if (avatarEl && (!avatarEl.getAttribute('src') || avatarEl.getAttribute('src') === '#')) {
  avatarEl.src = avatarImg;
}
...
// Utility: remove cards by names from DOM (non-destructive)
// Usage in DevTools: window.removeCardsByNames(['grger','Мадрид'])
// Matching is case-insensitive and will check if the card title equals or contains the provided string.
export function removeCardsByNames(names = []) {
  if (!Array.isArray(names) || names.length === 0) return 0;
  const container = document.querySelector('.elements__list');
  if (!container) return 0;
  const normalized = names.map(n => String(n).trim().toLowerCase()).filter(Boolean);
  const items = Array.from(container.querySelectorAll('.elements__list-item'));
  let removed = 0;
  items.forEach(item => {
    const titleEl = item.querySelector('.elements__text');
    if (!titleEl) return;
...
// expose for quick access from DevTools
window.removeCardsByNames = removeCardsByNames;
